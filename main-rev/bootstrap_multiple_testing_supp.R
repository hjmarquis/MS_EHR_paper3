...
name.list = paste0(c("RN","DF"),"_MStrt")
out.name.list = paste0("low-d-data-",name.list)
...
{
  dat.name = paste0("comb-data-",name)
  out.name = paste0("low-d-data-",name)
  load( paste("MS CLIME analysis/data-analysis/",dat.name,"p10_yr.rda",sep=''))
  Z = Z[,1:(grep("PheCode",colnames(Z))[1]-1)]
  ...
  save(
    ...
    boot.est, file = paste0("MS CLIME analysis/result/MS_result_new_",out.name,"p10_boot_joint.rda"))
...
load(paste0("MS CLIME analysis/result/MS_result_new_",out.name.list[i],"p10_boot_joint.rda"))
